  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex justify-between text-xs text-gray-500">
        <span className={overLines ? 'text-red-500' : ''}>{lineCount} 行{overLines && ` (最多 ${MAX_LINES} 行)`}</span>
        <span className={overChars ? 'text-red-500' : ''}>{charCount} 字{overChars && ` (最多 ${MAX_CHARS} 字)`}</span>
      </div>
    </div>
  );
}
