...
export function render(input: RenderInput): void {
  ...
  for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
    ...
    for (let i = 0; i < line.length; i++) {
      ...
      const charX = lineCenterX - (ctx.measureText(line).width / 2) + ctx.measureText(line.substring(0, i + 1)).width - ctx.measureText(ch).width / 2;
      ...
      if (animationState.scale !== 1 || charRotate !== 0) {
        ...
        if (style.stroke.enabled) {
          ctx.lineWidth = style.stroke.width;
          ctx.strokeStyle = style.stroke.color;
          ctx.strokeText(ch, charX, lineY + charOffset);
        }
        ctx.fillStyle = animationState.colorOverride || style.color;
